...
- 修复「启动器偶尔自己消失」：原先的卡顿取证代码会长期、高频地转储所有线程的调用栈，在 Windows + Python 3.13 上偶发踩到解释器自身的缺陷，进程直接退出，连崩溃日志都来不及写。现在改为只在检测到界面真的卡住时取样一次，平时完全不做转储。
- 新增两份「出事才写」的诊断日志：`data/fault.log`（致命错误时的全线程调用栈）和 `data/thread-errors.log`（后台线程 / Qt 回调里没人接的异常）。

**插件报错提示**

- 认得出插件加载失败：除原有日志形态外，现在也能识别 ComfyUI 自己给出的权威提示（`IMPORT FAILED`、`Cannot import ... for custom nodes`），并给出干净的插件目录名，不再出现「插件明明报错了，启动器一声不吭」。
- 首页提示条自动换行，最多列前 3 个失败插件（超出显示「等 N 个」），不再把「一键启动」页顶变形；完整清单仍然写进控制台日志。
...
- 发布包纳入 Qt WebEngine 运行库，用于显示内置 ComfyUI 画布。

...
- 定位到一个会让启动器**静默退出**的问题：用于卡顿取证的 C 层看门狗会长期、高频地转储所有线程的调用栈，在 Windows / Python 3.13 上偶发触发解释器自身的缺陷（访问违例），进程直接消失、连崩溃日志都留不下。
- 现在改为**只在检测到界面真的卡住时取样一次**，日常使用不再有任何周期转储；同时新增两份"出事才写"的诊断日志（`data/fault.log`、`data/thread-errors.log`），万一再遇到异常退出，日志里会留下完整的线程调用栈。

**修复：插件加载失败提示**

- 之前只在特定形态的日志行上才能认出"某个插件加载失败"，ComfyUI 自己给出的权威提示（`IMPORT FAILED`、`Cannot import ... for custom nodes`）反而认不出来，常见情况是**插件报错了但启动器毫无提示**。现在这两类都会识别，并给出干净的插件目录名。
- 修好"插件报错挤成一行把界面撑宽"的问题：首页提示条现在会自动换行、只列前几个插件（超出显示"等 N 个"），不再把界面顶变形；完整清单仍然写进控制台日志。

**修复：ComfyUI 不再被启动器"带走"**

- 以前启动器崩溃或被强制关闭时，正在运行的 ComfyUI 也会一起消失（子进程挂在管道上，管道一断就报错退出；而且 Qt 会把子进程放进一个"父进程退出就清理"的作业对象里）。现在 ComfyUI 改用独立进程方式启动、输出写入日志文件，**启动器关了/崩了，ComfyUI 照常跑**，日志也留在 `data/comfyui-console.log` 里可查。

**修复：内置画布里"退出前的工作流"不再丢**

- ComfyUI 把"当前打开的工作流"存在浏览器存储里，只有画布宿主**优雅退出**时才会写盘。以前启动器关得急（或画布宿主还没就绪就被关）会直接强杀，存储来不及落盘，下次打开就是空白工作流。
- 现在关闭前一定先礼貌地请画布宿主自己退出并最多等 5 秒，只有它赖着不退才强杀；启动时也会清掉上一次残留的画布宿主（两个宿主同时写同一个浏览器配置会互相打架、照样丢工作流）。

**界面调整**

- 「打开网页」按钮从「一键启动」页移到「控制台」页（和「关闭 ComfyUI」放在一起）。

**改进**

- 模型管理页「C 站链接下载」加入后台后，界面会出现一条**进度条**（第 i/n 条 · 百分比 · 速度 · 剩余时间 · 成功/跳过/失败计数），旁边有 **「查看下载进度」**按钮可随时打开进度窗口：每条链接的状态、速度、剩余时间与实时日志，还能一键停止；**关掉窗口不影响下载**。
- 关闭 ComfyUI 后自动清空 ComfyUI 日志窗格（启动器自己的操作记录保留），下次启动是全新一屏。
- 环境管理页的「特殊安装」预设（Torch / Triton / SageAttention / xformers / llama-cpp）不再让界面卡住：环境探测改到后台线程，探过的预设第二次点是秒开。

**新增**

- 工具箱 → 小工具：新增 **「角色tag大全」**（animadex 角色 tag 检索）。
- 标题栏（应用名右侧）展示官方 QQ 群号 **756754216**，可用鼠标选中复制。

### v0.6 正式版 · 2026-09-13

**新增：内置 ComfyUI 画布**

- 新增「ComfyUI 画布」页，直接在启动器内加载完整 ComfyUI 节点界面，支持现有工作流、自定义节点与上传/生成操作，无需另开浏览器。
- 画布按需加载：首次进入该页时才启动内置网页渲染组件。

**界面调整**

- 启动成功后默认不再自动打开外部浏览器；设置页新增「使用内置 ComfyUI 画布」开关，可恢复原来的外部浏览器启动方式。
- 保留「外部浏览器」按钮，遇到特殊插件页面时可随时切回浏览器使用。
- GitHub 更新提示只显示正式版本状态，不再显示面向用户的本地开发版本提示。

**运行库**

- 发布包纳入独立的 Electron 画布宿主（不是 Qt WebEngine），用于显示内置 ComfyUI 画布。
